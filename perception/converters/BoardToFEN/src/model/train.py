...
from perception.converters.BoardToFEN.src.model.evaluate import eval_model
from perception.converters.BoardToFEN.src.model.model import PieceClassifier
from perception.converters.BoardToFEN.src.model.dataset import PiecesDataset
from perception.converters.BoardToFEN.src.utils.transforms import (
    parse_config_transforms,
)
from perception.converters.BoardToFEN.src.consts import (
    TRAIN_CONFIG_PATH,
    TRAIN_CONFIG_NAME,
)
from perception.converters.BoardToFEN.src.visualization.train_progress_visualization import (
    plot_learning_curves,
)


@hydra.main(
    config_path=TRAIN_CONFIG_PATH, config_name=TRAIN_CONFIG_NAME, version_base="1.2"
)
def train(
    config: DictConfig,
) -> (str, torch.utils.data.DataLoader, torch.utils.data.DataLoader):
    ...
    current_run_path = hydra.core.hydra_config.HydraConfig.get()["runtime"][
        "output_dir"
    ]
    ...
    train_dataset = PiecesDataset(
        images_dir_path=images_dir_path,
        labels_path=train_labels_path,
        transforms=transforms,
    )
    val_dataset = PiecesDataset(
        images_dir_path=images_dir_path,
    ...
    if is_minibatch:
        ...
        eval_train_loader = get_subset_dataloader(
            dataset=train_dataset, subset_ratio=config.hyperparams.train.eval_train_size
        )
        val_loader = None
    ...
    for epoch in range(num_epochs):
        ...
        for iter_num, data in enumerate(train_loader):
            ...
            if iter_num % print_interval == 0:
                ...
                if iter_num > 0:
                    log.info(
                        f"epoch: {epoch}, iteration: {iter_num}, loss: "
        ...
        epoch_train_accuracy, epoch_train_loss = eval_model(
            model,
            criterion,
            eval_train_loader,
            device=device,
        ...
        epoch_val_accuracy, epoch_val_loss = eval_model(
            model,
            criterion,
            eval_val_loader,
            device=device,
    ...
    if is_minibatch:
        plot_learning_curves(
            epoch_losses, epoch_train_accuracies, plots_path=plots_path
        )
    else:
        plot_learning_curves(
            epoch_losses,
...
def get_subset_dataloader(
    dataset: torch.utils.data.Dataset, subset_ratio: float
    ...
    random_indices = np.random.choice(
        np.arange(len(dataset)), size=subset_size, replace=False
    )
