...
from perception.converters.BoardToFEN.src.data.consts.piece_consts import LABELS
...
class PiecesDataset(Dataset):
    def __init__(
        ...
        self.image_path_labels_pairs = self.create_image_labels_pairs(
            images_dir_path, self.labels_dict
        )
    ...
    def create_image_labels_pairs(
        self, images_dir_path: str, labels_dict: Dict[str, Dict[str, str]]
        ...
        for idx, image_name in enumerate(labels_dict):
            ...
            path_label_pair = {"image": image, "label": one_hot_labels[idx]}
    ...
    def transform_labels(
        ...
        one_hot_label = F.one_hot(
            torch.Tensor(labels).to(torch.long), num_classes=len(LABELS)
        ).to(torch.float64)

        return one_hot_label

    @staticmethod
    def board_squares_to_pieces_dataset(
        board_squares: Dict[Tuple[int, int], np.ndarray], transforms: list = None
    ) -> torch.Tensor:
        ...
        if transforms:
            squares_pil_images = [
                Image.fromarray(square_array.astype(np.uint8))
                for square_array in squares
            ]
