...
from perception.converters.BoardToFEN.src.visualization.tensorboard_visualization_utils import (
    plot_classes_preds,
)
    ...
    with torch.no_grad():
        ...
        for i, (images, labels) in enumerate(loader):
            ...
            tb_writer.add_figure(
                f"Evaluation on {state}, epoch {epoch_num}, eval iteration {i}",
        ...
        if log:
            ...
            for piece_class, piece_name in REVERSED_LABELS.items():
                log.info(
                    f"Accuracy for {piece_name}: {class_accuracy[piece_class]:.3f}"
                )

            log.info("")
            log.info(
                f"Balanced class accuracy on {state}: {balanced_class_accuracy.item():.3f}"
            )
