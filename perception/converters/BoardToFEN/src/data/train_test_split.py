...
def train_test_split(config: DictConfig):
    ...
    with open(LABELS_OUTPUT_FILE_PATH) as file:
        ...
        for board_type_name, dict_of_squares_for_board_type in labels.items():
            for board_name, board_item in dict_of_squares_for_board_type.items():
                for square_item in board_item.items():
                    single_labels_list.append(
                        (board_type_name, (board_name, square_item))
                    )
        ...
        val_samples_idx = np.array(
            list(set(np.arange(num_samples)) - set(train_samples_idx))
        )

        create_subset_from_selected_indices(
            single_labels_list,
            selected_idx=train_samples_idx,
            board_types_names=labels.keys(),
            out_json_path=TRAIN_LABELS_OUTPUT_FILE_PATH,
        )
        create_subset_from_selected_indices(
            single_labels_list,
            selected_idx=val_samples_idx,
            board_types_names=labels.keys(),
            out_json_path=VAL_LABELS_OUTPUT_FILE_PATH,
        )


def create_subset_from_selected_indices(
    dataset: List[Tuple[str, dict]],
    selected_idx: np.ndarray,
    board_types_names: List[str],
    out_json_path: str,
):
