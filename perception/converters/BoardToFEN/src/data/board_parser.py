...
from perception.converters.BoardToFEN.src.data.labeler import get_piece_labels
from perception.converters.BoardToFEN.src.board.board import parse_board
from perception.converters.BoardToFEN.src.data.consts.squares_consts import (
    RELEVANT_SQUARES,
    BOARD_SIDE_SIZE,
)
from perception.converters.BoardToFEN.src.data.consts.path_consts import (
    DATA_DIR,
    DIRS_TO_PARSE_NAMES,
    PIECES_OUTPUT_DIR_PATH,
    LABELS_OUTPUT_FILE_PATH,
)
    ...
    with open(str(LABELS_OUTPUT_FILE_PATH), "w") as labels_json:
        ...
        for i, data_dir in enumerate(data_dirs):
            ...
            for file_name in os.listdir(data_dir):
                ...
                relevant_squares_dict = get_relevant_squares(
                    board_squares=board_squares_dict,
                    relevant_squares=RELEVANT_SQUARES[data_dir_name],
                )

                labeled_squares_dict = label_squares(
                    relevant_squares_dict, data_dir_name
                )

                labels_dict[data_dir_name][stripped_file_name] = save_squares(
                    labeled_squares_dict=labeled_squares_dict,
                    data_dir_index=i,
...
def get_relevant_squares(
    board_squares: Dict[Tuple[int, int], np.ndarray],
    relevant_squares: Dict[str, List[int]],
...
def label_squares(
    squares_dict: Dict[Tuple[int, int], np.ndarray], board_type: str
    ...
    for (i, j), square in squares_dict.items():
        ...
        labeled_squares_dict[(i, j)] = {"square": square, "label": square_label}

    return labeled_squares_dict


def save_squares(
    labeled_squares_dict: Dict[Tuple[int, int], Dict[str, Any]],
    data_dir_index: int,
