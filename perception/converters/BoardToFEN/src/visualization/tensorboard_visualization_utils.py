...
from perception.converters.BoardToFEN.src.data.consts.piece_consts import (
    REVERSED_LABELS,
)
from perception.converters.BoardToFEN.src.visualization.consts import (
    NUM_DISPLAYED_IMAGES_PER_ROW,
)
...
def plot_classes_preds(images: torch.Tensor, preds: torch.Tensor, labels: torch.Tensor):
    ...
    num_preds = len(preds)
    num_rows = (
        num_preds // NUM_DISPLAYED_IMAGES_PER_ROW + 1
        if num_preds % NUM_DISPLAYED_IMAGES_PER_ROW
    ...
    for idx in np.arange(num_preds):
        ax = axs[
            idx // NUM_DISPLAYED_IMAGES_PER_ROW, idx % NUM_DISPLAYED_IMAGES_PER_ROW
        ]
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    if num_rows * NUM_DISPLAYED_IMAGES_PER_ROW > num_preds:
        for idx in range(
            num_preds % NUM_DISPLAYED_IMAGES_PER_ROW, NUM_DISPLAYED_IMAGES_PER_ROW
        ):
...
def images_to_probs(preds: torch.Tensor):
    ...
    predicted_probs = [
        torch.softmax(pred_output, dim=0)[predicted_label].item()
