...
def canny_edge_detector(
    image: np.ndarray, high_thresh_ratio: float, low_thresh_ratio: float
) -> np.ndarray:
...
def non_max_suppression(
    edge_magnitude: np.ndarray, edge_angle: np.ndarray
) -> np.ndarray:
...
def double_edge_threshold(
    edge_image: np.ndarray, low_thresh: float, high_thresh: float
) -> np.ndarray:
    ...
    for i in range(1, height - 1):
        for j in range(1, width - 1):
            if thresholded[i, j] == Canny.weak_edge.value:
                if np.any(
                    thresholded[i - 1 : i + 2, j - 1 : j + 2] == Canny.strong_edge.value
                ):
