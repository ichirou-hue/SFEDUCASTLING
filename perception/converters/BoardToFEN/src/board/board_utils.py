...
from perception.converters.BoardToFEN.src.data.consts.squares_consts import (
    BOARD_SIDE_SIZE,
)


def crop_image(
    image: np.ndarray, edges: np.ndarray, verbose: bool
...
def split_board_image_to_squares(
    image: np.ndarray,
    cropped_row_seq: np.ndarray,
    cropped_col_seq: np.ndarray,
    board_side_size: int = BOARD_SIDE_SIZE,
    ...
    """
    board_square_vertices = convert_frame_to_square_vertices(
        cropped_row_seq, cropped_col_seq
    )
...
def convert_frame_to_square_vertices(
    row_seq: np.ndarray, col_seq: np.ndarray, board_size: int = BOARD_SIDE_SIZE
) -> np.ndarray:
    ...
    for i, (top_row_idx_margins, bottom_row_idx_margins) in enumerate(
        zip(x_edge_margins[:-1], x_edge_margins[1:])
    ):
        for j, (left_col_idx_margins, right_col_idx_margins) in enumerate(
            zip(y_edge_margins[:-1], y_edge_margins[1:])
        ):
...
def find_edges(
    seq: np.ndarray, split_size, margin_size: int = 1
) -> List[Tuple[int, int]]:
    ...
    frame_split_idx = [
        idx + fragment_size * i - start_pad_size
        for i, idx in enumerate(max_len_idx_per_row)
    ]

    # pad margins
    edges_margins = [
        (max(0, idx - margin_size), min(len(seq) - 1, idx + margin_size))
        for idx in frame_split_idx
    ]
...
def pad_fragments(fragments: List[np.ndarray]) -> Tuple[np.ndarray, int, int]:
    ...
    padded_fragments[0] = np.pad(
        padded_fragments[0], (pad_start, 0), mode="constant", constant_values=0
    )
    padded_fragments[-1] = np.pad(
        padded_fragments[-1], (0, pad_end), mode="constant", constant_values=0
    )
...
def get_max_seq_lens_per_row(frame: np.ndarray) -> np.ndarray:
    ...
    row_unique_seqs_number = [
        len(list(values)) for key, values in itertools.groupby(rows_number)
    ]
    ...
    seq_idx_list_per_row = [
        row_seq_indices.reshape(-1, 2) for row_seq_indices in seq_idx_list_per_row
    ]
    seq_lens_per_row = [
        np.diff(row_seq_indices) for row_seq_indices in seq_idx_list_per_row
    ]
