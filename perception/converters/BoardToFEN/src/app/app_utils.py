...
from perception.converters.BoardToFEN.src.app.consts import ActiveColor
from perception.converters.BoardToFEN.src.fen_converter.fen_converter import (
    convert_board_pieces_to_fen,
    convert_fen_to_url,
)


def gui_to_fen_parameters(
    board_rows_as_fen: List[str],
    current_color_index: int,
    castling_rights_checkboxes: List[BooleanVar],
    file_var: StringVar,
    row_var: StringVar,
    n_halfmoves_var: StringVar,
    n_fullmoves_var: StringVar,
    domain_number: int,
    default_value: str,
) -> str:
    ...
    castling_rights = [
        bool(checkbox_var.get()) for checkbox_var in castling_rights_checkboxes
    ]
    ...
    n_half_moves = (
        n_halfmoves_var.get() if n_halfmoves_var.get() != default_value else 0
    )
    n_full_moves = n_fullmoves_var.get()

    fen_parts = convert_board_pieces_to_fen(
