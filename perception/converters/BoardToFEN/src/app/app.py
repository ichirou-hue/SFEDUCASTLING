...
from perception.converters.BoardToFEN.src.pipeline.pipeline import Pipeline
from perception.converters.BoardToFEN.src.fen_converter.consts import Domains
from perception.converters.BoardToFEN.src.app.app_utils import (
    gui_to_fen_parameters,
    show_disappearing_message_box,
)
...
class App:
    ...
    def __init__(
        self,
        pipeline: Pipeline,
        active_color_image_paths: Dict[str, str],
        screenshot_image_path: str,
        domain_logo_paths: Dict[int, str],
    ):
        ...
        self.make_active_color_button_and_screen_shot_button(
            active_color_image_paths, screenshot_image_path
        )
        ...
        self.castling_rights_checkboxes = self.make_castling_availability_checkboxes(
            self.checkbox_texts
        )
    ...
    def make_active_color_button_and_screen_shot_button(
        self, active_color_image_paths: Dict[str, str], screenshot_image_path: str
    ):
        ...
        self.photo_images = [
            ...
            for image_path in active_color_image_paths.values()
        ]
        self.photo_images = [image.subsample(2) for image in self.photo_images]

        # Load the first image and create an image on the canvas
        bar_id = canvas.create_image(
            50, 25, anchor=tk.CENTER, image=self.photo_images[0]
        )
        ...
        popup_label = tk.Label(
            self.app, text="", bg="white", relief=tk.SOLID, borderwidth=1
        )
        popup_label.place_forget()

        # Make screenshot button
        button = tk.Button(
            self.app, image=screenshot_image, command=self.on_screenshot_click
        )
        ...
        canvas.tag_bind(
            bar_id, "<Button-1>", lambda event: self.change_image(canvas, bar_id)
        )
    ...
    def make_castling_availability_checkboxes(
        self, checkbox_texts: List[str]
    ) -> List[tk.BooleanVar]:
        ...
        for i in range(4):
            ...
            checkbutton = tk.Checkbutton(
                frame, text=checkbox_texts[i], variable=check_var
            )
    ...
    def make_enpassant_dropdowns(
        self, square_options: Dict[str, List[str]]
        ...
        reset_button = tk.Button(
            self.app, text="reset", command=lambda: reset_selections(file_var, row_var)
        )
    ...
    def make_halfmoves_dropdown(self, halfmove_options: List[str]):
        ...
        # Create the first dropdown
        halfmoves_combobox = ttk.Combobox(
            self.app,
            textvariable=halfmoves_var,
            values=halfmove_options,
            height=5,
            width=5,
            state="readonly",
        )
        ...
        # Create a Reset button
        reset_button = tk.Button(
            self.app, text="reset", command=lambda: reset_selections(halfmoves_var)
        )
    ...
    def make_fullmoves_entry(self):
        ...
        entry = tk.Entry(
            self.app,
            textvariable=fullmoves_entry_var,
            validate="key",
        ...
        # Create a Reset button
        reset_button = tk.Button(
            self.app, text="reset", command=lambda: reset_entry(fullmoves_entry_var)
        )
        reset_button.pack(padx=10)
    ...
    def make_domain_buttons(self, image_paths: Dict[int, str]):
        ...
        def create_image_button(image, button_number):
            button = tk.Button(
                self.app,
                image=image,
                command=lambda: self.on_domain_click(button_number),
            )
            button.image = image
    ...
    def show_status_message(self):
        ...
        if self.board_rows_as_fen is None:
            show_disappearing_message_box(
                app=self.app, message="Failed to detect board"
            )
        else:
            show_disappearing_message_box(
                app=self.app, message="Successfully analyzed board"
            )
    ...
    def on_domain_click(self, domain_number: int):
        ...
        """
        fen_url = gui_to_fen_parameters(
            board_rows_as_fen=self.board_rows_as_fen,
            ...
            n_halfmoves_var=self.n_halfmoves_var,
            n_fullmoves_var=self.n_fullmoves_var,
            domain_number=domain_number,
        )

        if (
            domain_number == Domains.chess.value
            or domain_number == Domains.lichess.value
        ):
            webbrowser.open(fen_url)
        elif domain_number == Domains.pure_fen.value:
            pyperclip.copy(fen_url)
            show_disappearing_message_box(
                app=self.app, message="Copied FEN to clipboard!"
            )
