...
from perception.converters.BoardToFEN.src.fen_converter.fen_utils import (
    convert_row_to_fen,
)
from perception.converters.BoardToFEN.src.fen_converter.consts import (
    ACTIVE_COLOR_MAPPING,
...
def convert_board_pieces_to_fen(
    active_color: bool,
    castling_rights: List[bool],
    possible_en_passant: Union[str, None],
    n_half_moves: Union[str, None],
    ...
    available_castling_rights_strings = [
        CASTLING_RIGHTS_MAPPING[idx] for idx in available_castling_rights_indices
    ]
    available_castling_rights = (
        "".join(available_castling_rights_strings)
        if available_castling_rights_strings
    ...
    if domain == Domains.chess.value or domain == Domains.lichess.value:
        url_parts = [
            ...
            domain_params["fen_parts_connector"].join(
                [domain_params["fen_rows_connector"].join(fen_parts[0]), *fen_parts[1:]]
            ),
