...
def convert_row_to_fen(row: np.ndarray) -> str:
    ...
    """
    start_indices, sequence_lengths = find_sequence_indices(
        row, value_to_count=LABELS["X"]
    )
...
def find_sequence_indices(
    arr: np.ndarray, value_to_count: int
    ...
    for is_desired_sequence, sequence_iterator in groupby(
        enumerate(arr == value_to_count), key=lambda x: x[1]
    ):
